...
void Game::initGame() {
    ...
    if (!planetTexture.loadFromFile("Graphics/planet.png")) {
    ...
    if (!planetTexture.loadFromFile("Graphics/planet.png")) {
        std::cerr << "Planet texture failed to load!" << std::endl;
    }

    planets.emplace_back(sf::Vector2f(400, 300), 40.0f, 800.0f, planetTexture);
    planets.emplace_back(sf::Vector2f(1200, 700), 60.0f, 1200.0f, planetTexture);
    planets.emplace_back(sf::Vector2f(800, 200), 30.0f, 600.0f, planetTexture);

    // Hindernisse hinzufügen
    obstacles.emplace_back(sf::Vector2f(500, 500), 50.0f);
    obstacles.emplace_back(sf::Vector2f(1000, 400), 70.0f);
...
void Game::updateGame() {
    ...
    for (const auto& planet : planets) {
        sf::Vector2f direction = planet.getPosition() - player.getPosition();
        float distance = std::sqrt(direction.x * direction.x + direction.y * direction.y);

        if (distance > minDistance) {
            float force = (gravitationStrength * planet.getMass()) / (distance * distance);
            force *= deltaTime;
            sf::Vector2f gravity = direction / distance * force;
            velocity += gravity;
        }
    }
    ...
    for (const auto& obstacle : obstacles) {
        ...
        if (playerBounds.intersects(obstacleBounds)) {
            // Geschwindigkeit reduzieren und zurücksetzen
            velocity *= -bounceDamping;
            player.move(-velocity * deltaTime * 2.0f); // Rückstoß
        }
    }
